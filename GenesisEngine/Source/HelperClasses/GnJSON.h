...
public:
	...
	int GetInt(const char* name, int defaultValue = -1);
	float GetFloat(const char* name, float defaultValue = -1.0f);
	bool GetBool(const char* name, bool defaultValue = false);
	const char* GetString(const char* name, const char* defaultValue);
	math::float2 GetFloat2(const char* name, float2 defaultValue = float2(0,0));
	math::float3 GetFloat3(const char* name, float3 defaultValue = float3(0,0,0));
	math::float4 GetFloat4(const char* name, float4 defaultValue = float4(0,0,0,0));
	Quat GetQuaternion(const char* name, Quat defaultValue = Quat(0,0,0,1));
	Color GetColor(const char* name, Color defaultValue = Color(0, 0, 0, 1));
